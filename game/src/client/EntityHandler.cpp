  ...
  for (auto &player : this->players) {
    ...
    if (player.getState() == 1 && player.health >= 0)
      DrawRectangle(
          ...
          player.getBoombermanPos()[1] * map->offset + map->start_y + 3,
          map->size - 6, map->size - 6, player.color);
  }
}
...
void EntityHandler::drawBombs(Map *map) {
  ...
  for (auto &bomb : this->bombs) {
    if (!bomb.ShouldExplode()) {
      ...
      DrawRectangle(bomb.pos_x * map->offset + map->start_x + 7,
                    bomb.pos_y * map->offset + map->start_y + 7, map->size - 14,
                    map->size - 14, c);
    } else
      this->explodeBomb(&bomb, map);
  }
}
...
int EntityHandler::explodeBomb(Bomb *bomb, Map *map) {
  ...
  auto it = std::find_if(this->bombs.begin(), this->bombs.end(),
                         [x, y, fuze_time](Bomb &bomb) {
                           return bomb.pos_x == x && bomb.pos_y == y &&
                                  bomb.plant_time == fuze_time;
                         });
  if (it != this->bombs.end()) {
    tilesExploded = bomb->boom(map);
    this->theFloorIsLava.insert(this->theFloorIsLava.begin(),
                                tilesExploded.begin(), tilesExploded.end());
    this->bombs.erase(it);
...
void EntityHandler::drawFire(Map *map) {
  for (auto tile : this->theFloorIsLava) {
    DrawRectangle(tile.x * map->offset + map->start_x,
                  tile.y * map->offset + map->start_y, map->size, map->size,
                  RED);
  }
}
...
void EntityHandler::tryExtinguish() {
  for (size_t i = 0; i < this->theFloorIsLava.size(); i++) {
    if (this->theFloorIsLava[i].shouldCalmDown())
      this->theFloorIsLava.erase(this->theFloorIsLava.begin() + i);
  }
}
