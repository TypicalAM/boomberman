    ...
    while (!WindowShouldClose()) {
        ...
        if(local_boomberman->pseudonim_artystyczny_według_którego_będzie_się_identyfikował_wśród_społeczności_graczy == true_local_name){
            if (IsKeyPressed(KEY_SPACE)) {
                ...
                serverHandler.conn->SendIPlaceBomb(local_boomberman_position[0],local_boomberman_position[1]);
            }
            if (IsKeyPressed(KEY_RIGHT)) {
                if(local_boomberman->move(&map, local_boomberman_position, 1, 0)){
                    serverHandler.conn->SendIMove(local_boomberman_position[0]+1, local_boomberman_position[1]);
                }
            }
            ...
            if (IsKeyPressed(KEY_UP)) {
                if(local_boomberman->move(&map, local_boomberman_position, 0, -1)) {
                    serverHandler.conn->SendIMove(local_boomberman_position[0], local_boomberman_position[1]-1);
                }
            }
